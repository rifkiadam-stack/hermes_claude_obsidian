...
1. Resolve a user vault. Prefer `--vault`, then `HERMES_OBSIDIAN_VAULT` (legacy: `CLAUDE_OBSIDIAN_VAULT`), then
...
5. Run `python "$CORE" transaction inspect BUNDLE --vault VAULT`.
...
7. Pass the inspect result's exact `approval_sha256`, which binds the expanded
   ...
   vault:
   `python "$CORE" transaction apply BUNDLE --vault VAULT --approved-plan-sha256 HASH`.
...
```bash
python "$CORE" checkpoint OPERATION_ID --vault VAULT \
  --as-of YYYY-MM-DD
```
