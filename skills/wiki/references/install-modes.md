...
`wiki skill`. Plugin assets live in an ephemeral cache. Mutable vault
state must resolve from the project, `--vault`, or workspace configuration,
never `${CLAUDE_PLUGIN_ROOT}`.

## Local Hermes skill package development
...
```bash
python "$CORE" init /path/to/vault \
...
```bash
python "$CORE" adopt /path/to/vault \
