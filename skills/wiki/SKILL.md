---
name: wiki
description: Route vault setup, adoption, and wiki workflows.
version: 1.0.0
...
metadata:
  hermes:
    tags: [wiki, obsidian, vault, orchestration]
    related_skills: [wiki-ingest, wiki-query, save, wiki-lint, wiki-fold, autoresearch]
---

# Wiki orchestration

Treat the installed product as code and the selected user vault as data. Never use
the product root as a vault, even when the current directory happens to be
...
`HERMES_OBSIDIAN_VAULT` (legacy: `CLAUDE_OBSIDIAN_VAULT`), the nearest `.claude-obsidian.json`, then an
...
```bash
python "$CORE" init /absolute/path/to/vault \
...
```bash
python "$CORE" adopt /absolute/path/to/vault \
...
Hermes `write_file`/`patch` under the native Windows write policy tools or an Obsidian transport.
...
```bash
python "$CORE" checkpoint OPERATION_ID --vault /absolute/path/to/vault
```
