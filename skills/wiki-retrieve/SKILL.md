---
name: wiki-retrieve
description: Retrieve relevant vault chunks with BM25 ranking.
version: 1.0.0
...
metadata:
  hermes:
    tags: [retrieval, bm25, chunks, obsidian]
...
```bash
python "$PREFIX" --vault "$VAULT" --all --no-llm --peek
python "$PREFIX" --vault "$VAULT" --all --no-llm
python "$BM25" --vault "$VAULT" build
python "$RETRIEVE" --vault "$VAULT" "wiki" --top 1 --no-rerank --explain
```
...
```bash
python "$RETRIEVE" --vault "$VAULT" "$QUERY" --top 5 --no-rerank --explain
```
...
```bash
python "$BM25" --vault "$VAULT" stats
python "$BM25" --vault "$VAULT" query "$QUERY" --top 10
python "$RERANK" --vault "$VAULT" "$QUERY" --peek
python "$RERANK" --vault "$VAULT" "$QUERY" --model nomic-embed-text --peek
```
