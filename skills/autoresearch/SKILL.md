---
name: autoresearch
description: Bounded source-grounded research that files a dossier.
version: 1.0.0
...
metadata:
  hermes:
    tags: [research, web, citations, dossier]
    related_skills: [wiki, wiki-ingest, save]
---
...
current-directory discovery. Never write into the product root.
...
```bash
python "$CORE" transaction inspect /path/to/research-bundle.json --vault /path/to/vault
# Set APPROVAL_SHA256 to the inspect result's approval_sha256 after review.
python "$CORE" transaction apply /path/to/research-bundle.json --vault /path/to/vault \
  --approved-plan-sha256 "$APPROVAL_SHA256"
```

Do not use Hermes `write_file`/`patch` under the native Windows write policy, Obsidian transport writes, deprecated locks, or
worker applies.
