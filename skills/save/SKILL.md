---
name: save
description: Save explicit selections to the Second Brain vault.
version: 1.0.0
author: AgriciDaniel (claude-obsidian upstream), Hermes Agent
license: MIT
platforms: [windows]
metadata:
  hermes:
    tags: [obsidian, save, second-brain, vault]
    related_skills: [wiki-query, wiki-ingest]
---

# Save selected conversation knowledge

**Scope:** explicit persistence of specific conversation content only; never auto-capture.
...
1. Resolve the user vault by explicit `--vault`, then
   `HERMES_OBSIDIAN_VAULT` (legacy: `CLAUDE_OBSIDIAN_VAULT`), workspace config, then current-directory discovery.
   The product/product root is never a vault.
...
Never use Hermes `write_file`/`patch` under the native Windows write policy, Obsidian CLI writes, deprecated per-file locks, or
per-worker mutations for these vault changes.

## Preview and apply

```bash
python "$CORE" transaction inspect /path/to/save-bundle.json --vault /path/to/vault
# Set APPROVAL_SHA256 to the inspect result's approval_sha256 after review.
python "$CORE" transaction apply /path/to/save-bundle.json --vault /path/to/vault \
...
```bash
python "$CORE" checkpoint OPERATION_ID --vault /path/to/vault
```
