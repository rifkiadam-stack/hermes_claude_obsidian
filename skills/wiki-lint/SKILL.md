---
name: wiki-lint
description: Deterministic lint of vault health and provenance.
version: 1.0.0
...
metadata:
  hermes:
    tags: [lint, obsidian, health, provenance]
    related_skills: [wiki, wiki-query]
---

# Lint the wiki

**Scope:** deterministic lint only; does not reason broadly or repair.
...
```bash
python "$CORE" lint --vault "$VAULT"
python "$CORE" lint --vault "$VAULT" --format markdown
```
