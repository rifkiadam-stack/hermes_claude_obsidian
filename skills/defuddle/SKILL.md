---
name: defuddle
description: Clean a URL into readable, citable content.
version: 1.0.0
...
metadata:
  hermes:
    tags: [clean, url, extraction]
    related_skills: [wiki-ingest, save]
---
...
```bash
python "$CORE" capture external-plan url "HTTPS_URL"
```
...
```bash
python "$CORE" contracts --verify --capability defuddle --vault VAULT
```
