---
name: wiki-query
description: Read-only answers grounded in vault evidence.
version: 1.0.0
...
metadata:
  hermes:
    tags: [query, obsidian, read-only, evidence]
    related_skills: [wiki-lint, wiki-retrieve]
---

# Query the wiki

**Scope:** explicitly vault-scoped read-only answers; never general knowledge.
...
3. Check whether retrieval is verified:
   ...
   ```bash
   python "$CORE" contracts --vault "$VAULT" --verify --capability wiki-retrieve
   ```
