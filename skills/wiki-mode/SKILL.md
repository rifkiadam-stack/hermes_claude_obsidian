---
name: wiki-mode
description: Route vault work with LYT, PARA, or Zettelkasten modes.
version: 1.0.0
...
metadata:
  hermes:
    tags: [mode, methodology, para, zettelkasten]
    related_skills: [wiki]
---

# Route by vault methodology

**Scope:** methodology routing; does not save content or migrate notes.
...
```bash
python "$MODE_HELPER" --vault "$VAULT" get
python "$MODE_HELPER" --vault "$VAULT" config
python "$MODE_HELPER" --vault "$VAULT" route concept "Concept name"
python "$MODE_HELPER" --vault "$VAULT" route source "Source title"
```
...
4. Preview the canonical configuration transaction; the command records the
   ...
   ```bash
   python "$CORE" mode set "$MODE" --vault "$VAULT" \
