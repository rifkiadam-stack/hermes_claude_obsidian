...
description: Validate and author Obsidian Flavored Markdown syntax.
version: 1.0.0
...
**Scope:** Obsidian Flavored Markdown only, not for general markdown.
...
```bash
python "$CORE" transaction inspect "$BUNDLE" --vault "$VAULT"
python "$CORE" transaction apply "$BUNDLE" --vault "$VAULT" \
