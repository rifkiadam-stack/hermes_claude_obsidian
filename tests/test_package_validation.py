...
class PackageValidationTests(unittest.TestCase):
    CANONICAL = (
        ...
        "version: 1.0.0\n"
        "author: Sample Author, Hermes Agent\n"
        "license: MIT\n"
        "platforms: [windows]\n"
        "metadata:\n"
        "  hermes:\n"
    ...
    def test_minimal_portable_package_is_valid(self) -> None:
        with tempfile.TemporaryDirectory() as directory:
            ...
            self.fixture(
                root,
                frontmatter=self.CANONICAL,
            )
            self.assertTrue(validate_package(root)["ok"])

    def test_canonical_hermes_frontmatter_shape_is_enforced(self) -> None:
        with tempfile.TemporaryDirectory() as directory:
            root = Path(directory)
            self.fixture(root, frontmatter=self.CANONICAL)
            self.assertTrue(validate_package(root)["ok"])
            ...
            self.assertIn("missing_hermes_metadata", codes)
            ...
            codes = {item["code"] for item in validate_package(root)["findings"]}
            self.assertIn("missing_hermes_attribution", codes)
    ...
    def test_mcp_hook_fields_and_session_start_restrictions_match_official_schema(
        ...
        with tempfile.TemporaryDirectory() as directory:
            root = Path(directory)
            self.fixture(
                root, frontmatter=self.CANONICAL
            )
