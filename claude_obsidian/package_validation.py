            ...
            if not item:
                return None, [
                    _finding(
                        "invalid_yaml_scalar",
                        relative,
                        f"line {number} has an empty list item",
                    )
                ]
            if item[0] in YAML_PLAIN_FORBIDDEN_START or ":" in item:
                return None, [
                    _finding(
                        "invalid_yaml_scalar",
                        relative,
                        f"line {number} has an unsafe list item; quote each value",
                    )
                ]
            items.append(item)
        return items, []
...
def _frontmatter(
    ...
    hermes_block: dict[str, Any] | None = None
        ...
        if indent == 0:
            ...
            if key == "metadata":
                if "metadata" in values:
                    errors.append(
                        _finding("duplicate_frontmatter_key", relative, key)
                    )
                nested: dict[str, Any] = {}
                values[key] = nested
                hermes_block = None
            ...
            hermes_block = None
            continue
        if indent == 2 and stripped.startswith("hermes:"):
            ...
            if not isinstance(metadata, dict):
                errors.append(
                    _finding(
                        "nonportable_frontmatter",
                        relative,
                        f"line {number} hermes block requires a metadata section",
                    )
                )
                hermes_block = None
                continue
            if "hermes" in metadata:
                errors.append(
                    _finding("duplicate_frontmatter_key", relative, "hermes")
                )
            hermes_block = {}
            metadata["hermes"] = hermes_block
            continue
        if indent >= 4 and hermes_block is not None and ":" in stripped:
            key, value = stripped.split(":", 1)
            key = key.strip()
            parsed_value, scalar_errors = _frontmatter_scalar(
                value.strip(), relative, number
            ...
            if parsed_value is None:
                continue
            if key in hermes_block:
                errors.append(_finding("duplicate_frontmatter_key", relative, key))
            hermes_block[key] = parsed_value
            continue
        errors.append(
            _finding(
                "nonportable_frontmatter",
                relative,
                f"line {number} has unsupported nesting or indentation",
            )
        )
...
def _validate_skills(root: Path) -> list[dict[str, str]]:
    ...
    for path in paths:
        ...
        if not isinstance(author, str) or "Hermes Agent" not in author:
            errors.append(
                _finding(
                    "missing_hermes_attribution",
                    relative,
                    "author must credit Hermes Agent",
                )
            )
        ...
        if not isinstance(platforms, list) or "windows" not in platforms:
            errors.append(
                _finding(
                    "missing_windows_platform",
                    relative,
                    "platforms must list windows",
                )
            )
        ...
        if not isinstance(hermes_metadata, dict):
            errors.append(
                _finding(
                    "missing_hermes_metadata",
                    relative,
                    "metadata.hermes section is required",
                )
            )
        else:
            tags = hermes_metadata.get("tags")
            related = hermes_metadata.get("related_skills")
            ...
            if not isinstance(related, list):
                errors.append(
                    _finding(
                        ...
                        relative,
                        "metadata.hermes.related_skills must be a list",
                    )
                )
        text = path.read_text(encoding="utf-8")
